...
deMorgan¬∧¬ : {P Q : prop} → (¬ P) ∧ (¬ Q) → ¬ (P ∨ Q)
deMorgan¬∧¬ (p , q) (left x) = p x
deMorgan¬∧¬ (p , q) (right y) = q y
  
deMorgan¬∨¬ : {P Q : prop} → (¬ P) ∨ (¬ Q) → ¬ (P ∧ Q)
deMorgan¬∨¬ (left x) (p , q) = x p
deMorgan¬∨¬ (right y) (p , q) = y q
...
{- ¬ (∀ x:A. P x) ⇔ ∃ x:A . ¬ P x 
    No tengo candidato, no puedo probarlo
-}
deMorgan¬∀ : {A : Set}{P : A → prop} →
             ¬ ((x : A) → P x) → ∃ A (λ x → ¬ (P x))
deMorgan¬∀ x = {!   !}
...
RAA→TerEx : RAA → TerEx
RAA→TerEx nnpp = nnpp (λ x → x (right (λ y → x (left y))))
...
app¬¬ nnpq nnp nq = nnp λ p → nnpq λ npq → nq (npq p)
...
∨¬¬-2 : {P Q : prop} → ¬¬ P ∨ ¬¬ Q → ¬¬ (P ∨ Q) 
∨¬¬-2 (left nnp) npvq = nnp (λ p → npvq (left p))
∨¬¬-2 (right nnq) npvq = nnq (λ q → npvq (right q))
